...
function sendMessage() {
    ...
    const sqlDataBox = document.getElementById('sql-query');
    ...
    .then(data => {
        if (data.success) {
            ...
            // If there is query result, display it in a table
            if (data.query_result && data.query_result.length > 0) {
                // Clear the existing table, if any
                sqlDataBox.innerHTML = ''; 

                // Create a table element
                const table = document.createElement('table');
                table.classList.add('mysql-table');  // Optional: Add CSS class for styling

                // Add the table header
                const thead = document.createElement('thead');
                const headerRow = document.createElement('tr');
                Object.keys(data.query_result[0]).forEach(columnName => {
                ...
                // Append the table to the sqlDataBox
                sqlDataBox.appendChild(table);
            } else {
                sqlDataBox.textContent = data.message || "No results found.";
            }
        } else {
            // Handle errors
